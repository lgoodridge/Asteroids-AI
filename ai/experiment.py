...
def run_experiment():
    ...
    if not os.path.exists(experiment_dir):
        print("Initializing experiment directory '%s'...")
        ...
        print("========================================", log)
        print("=     STARTED ON %s" % current_time_str, log)
        print("========================================", log)
    ...
    # If it does, resume the previous experiment
        ...
        current_time_str = datetime.now().strftime("%m/%d/%Y %H:%M")
        print("========================================", log)
        print("=     CONTINUED ON %s" % current_time_str, log)
        print("========================================", log)
    ...
    for generation_idx in range(start_idx, settings.MAX_GENERATIONS+1):
        ...
        if generation_idx == settings.MAX_GENERATIONS:
            ...
            break
        if stagnation_idx >= settings.MAX_GENERATIONS_WITHOUT_PROGESS:
            _write_to_log(log, "Progress has stagnated. Ending Experiment.")
            break

        # Create, evaluate, and save the generation
        ...
        progress_threshold = best_fitness + settings.PROGRESS_IMPROVMENT_THRESHOLD
...
def _write_meta_file(filename, meta_dict):
    ...
    with open(filename, "w") as meta_file:
        json.dump(meta_file, meta_dict, indent=4)
